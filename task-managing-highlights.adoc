...
You can copy source files that Data Sense is scanning from any supported unstructured data source to a directory in a specific target destination location (https://docs.netapp.com/us-en/cloud-manager-sync/reference-sync-requirements.html#supported-sync-relationships[target locations that are supported by Cloud Sync^]). After the initial copy, any data changed in the files are synchronized based on the schedule that you configure.
...
. You'll need to select the Target system and then select (or create) the Data Broker you plan to use. Review the Cloud Sync requirements beginning with the link:https://docs.netapp.com/us-en/cloud-manager-sync/task-sync-quick-start.html[Quick Start description^].
