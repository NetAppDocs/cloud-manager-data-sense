...
Optionally, if you have an unassigned node-based license for Cloud Volumes ONTAP that you won't be using, you can convert it to a Cloud Data Sense license with the same dollar-equivalence and the same expiration date. link:task-managing-licenses.html#exchange-unassigned-node-based-licenses[Go here for details].
...
+
If your NetApp Support Site account isn't available from the drop-down list, link:task-adding-nss-accounts.html[add the NSS account to Cloud Manager^].
