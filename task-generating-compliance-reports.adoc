...
You can save the report on the local machine as a .CSV file; which can include up to 5,000 rows of data. If Data Sense is scanning database tables (structured data) and files (unstructured data), there can be up to two .CSV reports downloaded.
//(), or as a .JSON file that you export to an NFS Share (which can include an unlimited number of rows). If Data Sense is scanning database tables (structured data), files (unstructured data), and folders (unstructured data), there can be up to three .CSV or .JSON reports downloaded.
...
A dialog displays a message that the report is being downloaded.
...
*Note:* Information about Folders is not currently available in the reports.
